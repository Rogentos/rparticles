...
echo "Uploading ${BUILD_LOG} to bpr.bluepink.ro..."
ssh ${SSH_ARGS} entropy@bpr.bluepink.ro mkdir -p "${REMOTE_DIR}"
if [ "${?}" != "0" ]; then
...
chmod 640 "${tmp_path}"
scp ${SCP_ARGS} "${tmp_path}" entropy@bpr.bluepink.ro:"${REMOTE_DIR}"/
if [ "${?}" != "0" ]; then
	rm "${tmp_path}"
	exit 1
fi
rm "${tmp_path}"
scp ${SCP_ARGS} "${BUILD_LOG}" entropy@bpr.bluepink.ro:"${REMOTE_DIR}"/ || exit 1
